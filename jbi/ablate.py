...
def get_weight_2_drop(weights, percentage):
    ...
    num_samples = int(
        samples.shape[0] * percentage_interest
    )  # get relative amount to use
    to_null = sorted_sample[0:num_samples, 1].astype(
        np.int16
    )  # get indices of top n corelated
    # pdb.set_trace()
    return to_null  # these will be the items set to zero


def ablation_study(model, loader, mask_dict, conf=None, mode="demo"):
    layers_interest_names = find_conv_layers(model).keys()
    avg_contribs = list()
    for i, e in tqdm(
        enumerate(layers_interest_names), total=len(layers_interest_names)
    ):
        model = reload_model(conf)
        model.eval()
        layers_interest = find_conv_layers(model)
        contribs = get_layer_contribs(
            model, layers_interest, i, loader, mask_dict, mode
        )
        print(contribs)
...
def get_layer_contribs(
    model, layer_list, layer_idx, loader, mask_dict, mode, ground_truths, device="cuda"
):
    ...
    for i, e in enumerate(layer_list.keys()):
        ...
        for filt_k in layer_mask:
            layer_list[e].weight[filt_k].copy_(
                torch.zeros_like(layer_list[e].weight[filt_k])
            )
    if mode == "task":
...
def reload_model(model_conf):
    model = model_loader(model_conf)
