...
class ParamSweeper(): 
    ...
    def start(self): 
        ...
        match train_mode: 
            case 'adversarial': 
                search_space= {
                    'lambda': np.arange(0,1,0.1).tolist()
                }
            case _: 
                search_space= None
        if search_space: 
            ...
            study = optuna.create_study(storage=self.storage,study_name="my_study",load_if_exists=True,directions=self.direction,sampler=my_sampler,)
        else: 
            self.n_trials:int = self.conf['n_trials']
            study = optuna.create_study(storage=self.storage,study_name="my_study",load_if_exists=True,directions=self.direction)
        print(f"The number of trials is {self.n_trials}")
        study.optimize(self.do_optim,n_trials=self.n_trials,show_progress_bar=True,gc_after_trial=True)
    def do_optim(self,trial_obj):
        ...
        trainer = trainer_cls(model=model,conf=ov_conf,data_loaders=dls,tb_writter=None)
