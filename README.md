...
```bash 
 conda create --name jbi python=3.10.15
 ```
 - environment was developed using python3 3.9 and CUDA 12.2
 - Updated to use python 3.10
 - Install the code as a package py doing. This will also make it editable allowing users to add their own classes with
```
...
# Preparing datasets 
 ...
 - it can load PNG/jpg images supported by PIL 
 - it expects you to provide a config file with the following information 
    - data_path: absolute path to CSV file 
    - inside the CSV file, you should define a split column for train/val/test 
...
-  To train your baseline model, use the config file in "./config/train_base." 
```bash 
    python3 -m jbi.train --config_path config/train_base.json
```
- NOTE: some paths are hardcoded. You will need to change those 
- Bellow, I will annotate the same config file with comments on what each section does 
...
# Training the multi-task model 
    - config/train_two_task.json
    - follows the same logic as the single-task model 
    - make sure to define an adequate log_dir
```bash 
    python3 -m jbi.train --config_path config/train_two_task.json
```
# TCAV
 - it uses the dataset class single because the TCAV concept class expects a dataset object that only returns an image. 
 - modify config/tcav_explore.json  to point at your data and your model weights 
 - Run the following script: 
 ```bash
 python3 -m jib.tcav --config_path ./config/tcav_explore.json
 ``` 
 - We should get an image similar to:
...
# Training Debiased model 
 ...
 ```bash
 python3 -m jbi.train --config_path ./config/train_adv_tcav.json 
 ```
