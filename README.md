...
-  To train your own baseline mdoel you can use the config file in "./config/train_base" 
```bash 
    python3 -m jbi.train --config_path config/train_base.json
```
- NOTE: some paths are hardcoded you will need to change those 
- Bellow I will annotate the same config file with comments off what section does 
...
# TCAV 
```bash 
    python3 -m jbi.train --config_path config/tcav_explore.json
```
#  Observing TCAV in model layers 
 - modify config/tcav_explore.json  to point at your data and your model weights 
 - Run the Following  script: 
 ```bash
 python3 -m jbi.tcav --config_path ./config/tcav_explore.json
 ``` 
